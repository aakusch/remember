...
// Integration guard: the chunker detects section boundaries by matching heading
// markers on the parser's flattened `plain` text. The parser used to strip the
// markers, so every chunk got an empty heading_path (and `#` comments inside fenced
// code were the ONLY lines that looked like headings). This test feeds the REAL
// parser output — not hand-written `#` markdown — so a regression is caught.
function headingPaths(md: string): string[][] {
  const parsed = createRemarkParser().parse(md);
  const chunks = createSmartSplitChunker({ size: 900, overlap: 0.15 }).chunk({
    plain: parsed.plain,
    ast: parsed.ast,
  });
...
describe('parser → chunker heading_path integration', () => {
  it('populates heading_path from real headings (nested)', () => {
  ...
  it('does NOT treat a `#` comment inside fenced code as a heading', () => {
    ...
    const paths = headingPaths(md);
    // Every chunk sits under the real heading; the code comment never created one.
